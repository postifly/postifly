...
export async function handleAdminParcelsPost(request: NextRequest) {
  ...
  try {
    ...
    if (!priceStr) {
      return NextResponse.json({ error: 'Item value is required' }, { status: 400 });
    }
    ...
    if (session.user.role === 'EMPLOYEE') {
      ...
      if (!employee?.employeeCountry || employee.employeeCountry !== expectedTariffCountry) {
        return NextResponse.json(
          {
            error:
    ...
    if (parsed.weight != null) {
      ...
      const currency = (
        tariff.currency || CURRENCY_BY_ORIGIN_ISO[tariffCountry] || 'GEL'
      ).toUpperCase();
      const nbgRates = await fetchNbgRates().catch(() => null);
      const converted = nbgRates && currency ? convertToGel(nbgRates, amount, currency) : null;
      shippingAmount = converted != null ? Math.round(converted * 100) / 100 : amount;
    }
    ...
    return NextResponse.json({ error: 'Error while creating parcel' }, { status: 500 });
  }
}
