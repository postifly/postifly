...
import { cachedDashboard, DashboardCacheTags } from '@/lib/cache/dashboardCache';
...
export async function GET() {
  ...
  try {
    const byFormCountry = await cachedDashboard(
      'tariffs:formCountryGel:v1',
      // same for all dashboard users (tariffs are global)
      { dest: 'GE' },
      async () => {
